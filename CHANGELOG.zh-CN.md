...
### 新增
- **频道关注与自动下载** - 关注 YouTube 频道，浏览视频，批量选择下载。后台轮询检测新上传并发送桌面通知。系统托盘支持关闭时最小化
- **频道下载设置完整对齐** - 频道下载现在使用与 YouTube 页面相同的完整设置：视频编解码器选择（H.264/VP9/AV1/Auto）、音频比特率、字幕、嵌入元数据/缩略图、SponsorBlock、速度限制。设置从共享配置同步
- **高分辨率智能编解码器选择** - 8K 默认使用 AV1，4K/2K 默认使用 VP9（编解码器设为 Auto 时）。选择高分辨率画质但未安装 FFmpeg 时显示安装对话框
- **8K 画质选项** - 频道下载现在支持 8K（4320p）画质，与 YouTube 页面一致
- **每频道新视频徽章** - 每个已关注频道显示未观看新视频数量徽章
- **可折叠的关注频道面板** - 右侧关注频道列表可通过切换按钮折叠/展开

### 修复
- **选择 4K 时频道下载卡在 1080p** - 视频编解码器被硬编码为 H.264，而 YouTube 在 1080p 以上没有 H.264。现在默认使用 Auto 编解码器，配合智能回退链
- **页面加载时格式下拉框为空** - 从 localStorage 加载的设置可能是音频格式（mp3），而 UI 显示视频格式选项。现在从保存的设置正确初始化音频/视频模式
