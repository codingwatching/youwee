...
### 新增
- **字幕工坊** - 功能齐全的字幕编辑器，可替代 Subtitle Edit。支持 SRT、VTT 和 ASS 格式，采用虚拟化编辑器确保大文件流畅运行。包含撤销/重做（50 级）、多选、内联编辑和键盘快捷键
- **字幕时间轴工具** - 对所有或选中的字幕进行时间偏移、按比例缩放时长、以及两点同步。支持毫秒级精度调整
- **字幕查找替换** - 在字幕文本中搜索，支持区分大小写、全词匹配和正则表达式。可单个替换或全部替换
- **字幕自动修复** - 检测并修复常见字幕问题：空条目、时间戳重叠、听障标签、过长行、重复条目、多余格式标签、以及过短/过长时长
- **从 URL 下载字幕** - 使用 yt-dlp 的字幕提取功能从任何视频 URL 获取可用字幕，支持自动生成字幕检测
- **AI 字幕翻译** - 使用已配置的 AI 提供商（Gemini、OpenAI、DeepSeek、Qwen、Ollama）将字幕翻译为任何语言。每批处理 20 条以确保稳定性
- **AI 语法修正** - 修正字幕中的语法和拼写错误，提供三种语气选择：保持原样、正式和口语化。由已配置的 AI 提供商驱动
- **Whisper 字幕生成** - 使用 OpenAI Whisper 从音视频文件生成字幕，直接集成在字幕工坊中
- **后端 AI 响应命令** - 新增 `generate_ai_response` Tauri 命令，读取已保存的 AI 配置并调用相应提供商，使前端 AI 功能得以实现
